...
const submitForm = data => {
  console.log(data); //formEl.submit();
};

const validateFld = (evt, condition) => {
  if (condition) {
    canSubmit = true;
...
emailFld.addEventListener('keyup', validateFld.bind(this, evt, emailRegEx.test(evt.target.value)));
...
fullnameFld.addEventListener('keyup', function (evt) {
  evt.preventDefault();
  validateFld(this, evt.target.value !== '');
});
formEl.addEventListener('submit', function (evt) {
  evt.preventDefault();
  const getFormValues = [...evt.target.elements].filter(el => el.type !== 'submit' && el).map(el => {
    return {
